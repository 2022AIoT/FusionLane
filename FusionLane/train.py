...
parser.add_argument('--model_dir', type=str, default='./my_model',
...
parser.add_argument('--end_learning_rate', type=float, default=5e-7,
...
def get_filenames(is_training, data_dir):
  ...
  """
  if is_training:
    return [os.path.join(data_dir, 'train-00000-of-00004.tfrecord'),
            os.path.join(data_dir, 'train-00001-of-00004.tfrecord'),
            os.path.join(data_dir, 'train-00002-of-00004.tfrecord'),
            os.path.join(data_dir, 'train-00003-of-00004.tfrecord')]
  else:
    return [os.path.join(data_dir, 'testing-00000-of-00004.tfrecord'),
            os.path.join(data_dir, 'testing-00001-of-00004.tfrecord'),
            os.path.join(data_dir, 'testing-00002-of-00004.tfrecord'),
            os.path.join(data_dir, 'testing-00003-of-00004.tfrecord')]
...
def preprocess_image(image, label, is_training):
  ...
  global CP_X, CP_Y, RD_SCALE, CP_x, CP_y
  #cp_x = math.floor(CP_X*RD_SCALE)
  #cp_y = math.floor(CP_Y*RD_SCALE)
  if is_training:
    # Randomly scale the image and label.
    """ image, label = preprocessing.random_rescale_image_and_label(
        image, label, _MIN_SCALE, _MAX_SCALE,RD_SCALE) """

    # Randomly crop or pad a [_HEIGHT, _WIDTH] section of the image and label.
    image, label = preprocessing.random_crop_or_pad_image_and_label(
        image, label, _HEIGHT, _WIDTH, _IGNORE_LABEL,CP_x,CP_y,RD_SCALE)
    ...
    image, label = preprocessing.random_crop_or_pad_image_and_label(
        image, label, _HEIGHT, _WIDTH, _IGNORE_LABEL,34,34,RD_SCALE)

  image.set_shape([_HEIGHT, _WIDTH, 4])
  label.set_shape([_HEIGHT, _WIDTH, 1])

  tmp = image[:,:,0:-1]
  region = image[:,:,-1:]
  tmp = preprocessing.mean_image_subtraction(tmp)
  image = tf.concat([tmp,region],2)
...
def main(unused_argv):
  ...
  for _ in range(FLAGS.train_epochs // FLAGS.epochs_per_eval):
    global CP_X, CP_Y, RD_SCALE,CP_x,CP_y
    #RD_SCALE = round(random.uniform(_MIN_SCALE,_MAX_SCALE),1)
    RD_SCALE = 1
    if RD_SCALE * 400 < 321:
      CP_X = 0
      CP_Y = 0
    else:
     CP_X = randint(0,math.floor(RD_SCALE*400 - 322))
     CP_Y = randint(0,math.floor(RD_SCALE*400 - 322))
    CP_x = randint(0,78)
    CP_y = randint(0,78)
    tensors_to_log = {
    ...
    model.train(
        input_fn=lambda: input_fn(True, FLAGS.data_dir, _BATCH_SIZE, FLAGS.epochs_per_eval),
        hooks=train_hooks,
        # steps=1  # For debug
    )
